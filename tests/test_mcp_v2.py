...
class McpV2Tests(unittest.TestCase):
 ...
 def test_cycle_reaches_fixpoint_and_order_is_deterministic(self):
  path=self.manifest({"version":"1.0","tools":[{"name":"b","capabilities":["NetworkAccess"]},{"name":"a","labels":["PII"]}],"flows":[["a","b"],["b","a"]]});out=io.StringIO();self.assertEqual(2,run(["mcp","scan",path,"--format","json"],out,io.StringIO()));payload=json.loads(out.getvalue());self.assertEqual(["b"],[item["tool"] for item in payload["findings"]]);self.assertEqual(2,payload["summary"]["flows"])
