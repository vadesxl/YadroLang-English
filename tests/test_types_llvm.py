...
class TypeAndLlvmTests(unittest.TestCase):
    ...
    def test_bool_literals_and_verified_ir(self):
        ir=compile("fn main() { if true { return 1 } else { return 0 } }")
        module=llvm.parse_assembly(ir); module.verify()
    ...
    def test_string_printing_is_valid(self):
        module=llvm.parse_assembly(compile('fn main() { print("hello") return 0 }')); module.verify()
    def test_comparison_return_is_abi_normalized(self):
        module=llvm.parse_assembly(compile("fn main() { return 2 > 1 }")); module.verify()
