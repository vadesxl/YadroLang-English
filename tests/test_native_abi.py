...
class NativeAbiTests(unittest.TestCase):
 ...
 def test_native_external_link_and_run(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   tmp=Path(tmp);obj=tmp/"program.obj";build_native(compile(source),str(obj));stubs=tmp/"runtime.c";stubs.write_text("#include <stdint.h>\n"+f"int64_t {external_symbol('user.data')}(void){{return 41;}}\n"+f"int64_t {external_symbol('anonymize')}(int64_t x){{return x+1;}}\n"+f"int64_t {external_symbol('net.send')}(int64_t x){{return x;}}\n",encoding="utf-8");exe=tmp/("app.exe" if os.name=='nt' else "app");command=[cc,str(obj),str(stubs),"-o",str(exe)];command[1:1]=["-fuse-ld=lld"] if os.name=='nt' else [];link=subprocess.run(command,capture_output=True,text=True);self.assertEqual(0,link.returncode,f"command={command}\nstdout={link.stdout}\nstderr={link.stderr}");result=subprocess.run([str(exe)],capture_output=True,text=True);self.assertEqual(0,result.returncode,f"stdout={result.stdout}\nstderr={result.stderr}");self.assertIn("42",result.stdout)
