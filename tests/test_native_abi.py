...
class NativeAbiTests(unittest.TestCase):
 def test_symbols_are_c_compatible_and_collision_resistant(self):
  names=[external_symbol(x) for x in ("user.data","user_data","пользователь.данные")]
 ...
 def test_native_external_link_and_run(self):
  ...
  with tempfile.TemporaryDirectory() as tmp:
   tmp=Path(tmp);obj=tmp/"program.o";build_native(compile(source),str(obj))
   ...
   exe=tmp/("app.exe" if __import__('os').name=='nt' else "app")
   subprocess.run([cc,str(obj),str(stubs),"-o",str(exe)],check=True,capture_output=True,text=True)
