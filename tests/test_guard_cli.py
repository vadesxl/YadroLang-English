...
class GuardCliTests(unittest.TestCase):
    ...
    def test_json_policy_violation_and_exit_code(self):
        ...
        code = run(["scan", path, "--format", "json"], out, err)
        self.assertEqual(EXIT_POLICY, code)
        item = json.loads(err.getvalue())
        self.assertEqual("YADRO-E2301", item["code"])

    def test_sarif_output_is_21(self):
        path = self.source("fn main() requires [NetworkAccess] { return net.send(user.data()) }")
        out, err = io.StringIO(), io.StringIO()
        code = run(["scan", path, "--format", "sarif"], out, err)
        self.assertEqual(EXIT_POLICY, code)
        self.assertEqual("2.1.0", json.loads(err.getvalue())["version"])

    def test_valid_program_exits_zero(self):
        path = self.source("fn main() { return 0 }")
        self.assertEqual(EXIT_OK, run(["scan", path], io.StringIO(), io.StringIO()))

    def test_invalid_policy_has_source_exit(self):
        with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as handle:
            handle.write('{"version":"9"}')
            path = handle.name
    ...
    def test_custom_policy_is_applied(self):
        ...
        self.assertEqual(EXIT_POLICY,
                         run(["scan", source, "--policy", str(policy)], io.StringIO(), io.StringIO()))
