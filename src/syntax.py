# -*- coding: utf-8 -*-
"""YadroLang AST and Pratt parser."""
...
class Parser:
    ...
    def parse(self):
        ...
        return program
    def _function(self):
        line=self._eat(Kind.FN).string; name=self._eat(Kind.NAME).text; self._eat(Kind.LPAREN); parameters=[]
    ...
    def _statement(self):
        ...
        if kind == Kind.RETURN:
            line=self._eat().string; return Return(line,self._expression())
        if kind == Kind.LET:
            line=self._eat().string; name=self._eat(Kind.NAME).text; self._eat(Kind.ASSIGN); return Let(line,name,self._expression())
        ...
        if kind == Kind.NAME and self.i+1 < len(self.t) and self.t[self.i+1].kind == Kind.ASSIGN:
            line=self._current().string; name=self._eat().text; self._eat(); return Assign(line,name,self._expression())
    ...
    def _expression(self, minimum=0):
        ...
        try:
            ...
            while True:
                ...
                operator=self._eat(); right=self._expression(precedence+1); left=Binary(operator.string,OP_TEXT[kind],left,right)
            return left
    ...
    def _primary(self):
        ...
        if token.kind == Kind.LPAREN:
            self._eat(); value=self._expression(); self._eat(Kind.RPAREN); return value
        if token.kind == Kind.NAME:
            ...
            if self._current().kind == Kind.LPAREN:
                ...
                while self._current().kind != Kind.RPAREN:
                    ...
                    if self._current().kind == Kind.COMMA: self._eat()
                self._eat(Kind.RPAREN); return Call(token.string,token.text,arguments)
